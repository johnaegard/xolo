...
static unsigned char tank_rotation = 0;
...
static void tank_update_addr(void) {
  unsigned long addr = TANK_SPRITE_VRAM + (unsigned long)tank_rotation * 32UL;
...
void tank_rotate_cw(void) {
  tank_rotation = (tank_rotation + 1) & 7;
  tank_update_addr();
}

void tank_rotate_ccw(void) {
  tank_rotation = (tank_rotation + 7) & 7;
  tank_update_addr();
}
